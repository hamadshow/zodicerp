...
export default function ProfitLossByCustomer() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const response = await apiService.get(`/reports/profit-loss-customer?start_date=${startDate}&end_date=${endDate}`);
      setData(response.data.main);
    } catch (error) {
      console.error('Failed to fetch profit & loss by customer:', error);
    } finally {
      setLoading(false);
    }
  }, [startDate, endDate]);
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'P&L by Customer');
    XLSX.writeFile(workbook, `Profit_Loss_by_Customer_${startDate}_to_${endDate}.xlsx`);
  };
