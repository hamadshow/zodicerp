...
export default function PurchasesReturn({ returns, suppliers, invoices, products, units, warehouses, purchaseInvoices = [], filters = {} }) {
    ...
    const t = (key, fallback) => {
        return translations[key] || translations[`purchases_return.${key}`] || translations[`common.${key}`] || fallback;
    };
    ...
    const breadcrumbs = [
        ...
        { label: t('sidebar.purchase_returns', 'Purchase Returns') }
    ];
    ...
    const statsContent = mode === 'list' && (
        <div className="stats-cards">
            <div className="stat-card">
                <div className="stat-icon purchase-return__stat-icon purchase-return__stat-icon--primary">
            ...
            <div className="stat-card">
                <div className="stat-icon purchase-return__stat-icon purchase-return__stat-icon--warning">
            ...
            <div className="stat-card">
                <div className="stat-icon purchase-return__stat-icon purchase-return__stat-icon--success">
            ...
            <div className="stat-card">
                <div className="stat-icon purchase-return__stat-icon purchase-return__stat-icon--info">
                    <span className="material-icons-outlined">checklist</span>
                </div>
                <div className="stat-content">
                    <div className="stat-value">
                        <span className="purchase-return__muted">{aggregateStats.draft}</span>
                        {' / '}
                        <span className="purchase-return__status-pill purchase-return__status-pill--pending">{aggregateStats.pending}</span>
                        {' / '}
                        <span className="purchase-return__status-pill purchase-return__status-pill--approved">{aggregateStats.approved}</span>
                        {' / '}
                        <span className="purchase-return__status-pill purchase-return__status-pill--completed">{aggregateStats.completed}</span>
                    </div>
    ...
    const tableColumns = useMemo(() => [
        {
            ...
            sortable: true,
            width: '140px',
            render: (row) => <strong className="purchase-return__mono">{row.return_number || `#${row.id}`}</strong>,
        },
        {
            header: t('date', 'Date'),
            key: 'return_date',
        ...
        {
            header: t('purchase_invoice', 'Purchase Invoice'),
            key: 'invoice_id',
            sortable: true,
        ...
        },
        {
            ...
            width: '150px',
            render: (row) => (
                <span className="purchase-return__mono">
    ...
    const handleToolbarSearch = (searchText) => {
        router.get(getLocalizedRoute('admin.purchases.returns.index'), {
            ...filters,
            search: searchText,
            page: 1,
    ...
    const handleServerSort = (sortKey, sortDirection) => {
        ...
        params.page = 1;
        router.get(getLocalizedRoute('admin.purchases.returns.index'), params, {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    const invoiceOptions = useMemo(() => {
        return (purchaseInvoices || invoices || []).map(inv => ({
            value: String(inv.id),
    ...
    const supplierOptions = useMemo(() => {
        return (suppliers || []).map(s => ({
            value: String(s.id),
            label: s.name_en || s.name_ar || s.name || ''
        }));
    }, [suppliers]);
    ...
    const returnReasonOptions = useMemo(() => [
        ...
        { value: 'wrong_item', label: t('reason_wrong_item', 'Wrong Item Shipped') },
        { value: 'excess_quantity', label: t('reason_excess', 'Excess Quantity Ordered') },
        { value: 'quality_issue', label: t('reason_quality', 'Quality Issue') },
        { value: 'expired', label: t('reason_expired', 'Expired / Close to Expiry') },
    ...
    const loadInvoiceData = useCallback((invoiceId) => {
        ...
        const inv = (purchaseInvoices || invoices || []).find(i => String(i.id) === String(invoiceId));
        if (!inv) {
        ...
        setData('supplier_id', inv.supplier_id || '');
    ...
    }, [purchaseInvoices, invoices, setData]);
    ...
    const handleInvoiceChange = (invoiceId) => {
        ...
        if (!invoiceId) {
            if (currentInvoiceId && (hasItems || isEditingExisting)) {
                if (!confirm(t('confirm_clear_invoice', 'Clearing the Purchase Invoice will remove all return lines and reset the document. Continue?'))) {
                    return;
                }
            }
            setData('invoice_id', '');
    ...
    const handleEdit = (ret) => {
        ...
        const validReturnReasons = ['damaged', 'defective', 'wrong_item', 'excess_quantity', 'quality_issue', 'expired', 'other'];
        ...
        const inv = (purchaseInvoices || invoices || []).find(i => String(i.id) === String(ret.invoice_id));
        const invDetailsMap = {};
        if (inv) {
        ...
        setData({
            ...ret,
            return_reason: validReturnReasons.includes(ret.return_reason) ? ret.return_reason : 'other',
        ...
        if (ret.invoice_id) {
            const invRec = (purchaseInvoices || invoices || []).find(i => String(i.id) === String(ret.invoice_id));
    ...
    const handleDelete = (id) => {
        if (confirm(t('delete_confirm', 'Are you sure you want to delete this return?'))) {
            destroy(getLocalizedRoute('admin.purchases.returns.destroy', { return: id }));
        }
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (!data.invoice_id) {
            alert(t('error_invoice_required', 'Please select a Purchase Invoice'));
            return;
        }
        if (!data.supplier_id) {
            alert(t('error_supplier_required', 'Supplier must be populated from the selected Purchase Invoice'));
            return;
        }
        if (!data.warehouse_id) {
            alert(t('error_warehouse_required', 'Warehouse must be populated from the selected Purchase Invoice'));
            return;
        }

        const validItems = data.items.filter(item => parseFloat(item.return_qty || 0) > 0);
        if (validItems.length === 0) {
            alert(t('error_items_required', 'Please set at least one return quantity greater than 0'));
            return;
        }
        ...
        if (mode === 'create') {
            post(getLocalizedRoute('admin.purchases.returns.store'), submissionOptions);
        } else {
            put(getLocalizedRoute('admin.purchases.returns.update', { return: data.id }), submissionOptions);
        }
    };
    ...
    const handleExportPDF = () => {
        ...
        const module = element.closest('.purchase-return');
        if (module) module.classList.add('generating-pdf');

        const opt = {
            margin: [5, 5],
            filename: `PurchaseReturn_${data.return_number || 'New'}.pdf`,
    ...
    const handleExportExcel = () => {
        ...
        XLSX.writeFile(workbook, `PurchaseReturn_${data.return_number || 'New'}.xlsx`);
    };
    ...
    const pageTitleText = isViewMode
        ? t('view_return', 'View Purchase Return')
        : mode === 'create'
            ? t('create_new_return', 'Create New Purchase Return')
            : t('edit_return', 'Edit Purchase Return');

    const printSupplierName = useMemo(() => {
        const s = suppliers?.find(x => String(x.id) === String(data.supplier_id));
        return s ? (s.name_en || s.name_ar || s.name || '-') : '-';
    }, [suppliers, data.supplier_id]);

    const printInvoiceNumber = useMemo(() => {
        const i = (purchaseInvoices || invoices || []).find(x => String(x.id) === String(data.invoice_id));
        return i ? (i.invoice_number || String(i.id)) : '-';
    }, [purchaseInvoices, invoices, data.invoice_id]);
    ...
    return (
        <AdminLayout activeMenu={t('sidebar.purchase_returns', 'Purchase Returns')}>
            <Head title={t('title', 'Purchase Returns Management')} />
            ...
            <BlankPage breadcrumbs={breadcrumbs} stats={mode === 'list' ? statsContent : undefined}>
                <div className="purchase-return">
                    ...
                    {showInvoiceChangeDialog && (
                        <div className="purchase-return__modal-backdrop" onClick={cancelInvoiceChange}>
                            <div className="purchase-return__modal" onClick={(e) => e.stopPropagation()}>
                                <div className="purchase-return__modal-icon purchase-return__modal-icon--warn">
                                    <span className="material-icons-outlined">warning</span>
                                </div>
                                <h3 className="purchase-return__modal-title">
                                    {t('confirm_change_invoice_title', 'Change Purchase Invoice?')}
                                </h3>
                                <p className="purchase-return__modal-text">
                                    {t('confirm_change_invoice_body', 'Changing the Purchase Invoice will remove all current return lines and reload data from the newly selected invoice. This action cannot be undone.')}
                                </p>
                                <div className="purchase-return__modal-actions">
                                    <button
                                        type="button"
                                        className="purchase-return__btn purchase-return__btn--ghost"
                                    ...
                                    <button
                                        type="button"
                                        className="purchase-return__btn purchase-return__btn--danger"
                    ...
                    {mode === 'list' ? (
                        <div className="fade-in">
                            <Table
                                ...
                                onPageChange={(page) => {
                                    router.get(getLocalizedRoute('admin.purchases.returns.index'), { ...filters, page }, {
                                        preserveState: true,
                                        preserveScroll: true,
                                        replace: true,
                                    });
                                ...
                                onRecordsPerPageChange={(perPage) => {
                                    router.get(getLocalizedRoute('admin.purchases.returns.index'), { ...filters, page: 1, per_page: perPage }, {
                                        preserveState: true,
                                        preserveScroll: true,
                                        replace: true,
                                    });
                        ...
                        <div className="fade-in">
                            <div className="purchase-return__card">
                                <div className="purchase-return__doc-header">
                                    <div className="purchase-return__header-group">
                                        <button type="button" className="purchase-return__back-btn" onClick={() => setMode('list')} disabled={processing}>
                                            <span className="material-icons-outlined">arrow_back</span>
                                            <span>{t('common.back', 'Back')}</span>
                                        </button>
                                        <div className="purchase-return__title-block">
                                            <h2 className="purchase-return__title">{pageTitleText}</h2>
                                            <p className="purchase-return__subtitle">{t('purchase_return_subtitle', 'Returns are always generated from a Purchase Invoice. Select an invoice to auto-populate all fields.')}</p>
                                        </div>
                                    </div>
                                    <div className="purchase-return__action-bar">
                                        {!isViewMode && (
                                            <>
                                                <button type="button" className="purchase-return__action-btn" onClick={handlePrint}>
                                                    <span className="material-icons-outlined">print</span>
                                                    <span>{t('print', 'Print')}</span>
                                                </button>
                                                <button type="button" className="purchase-return__action-btn" onClick={handleExportPDF}>
                                                    <span className="material-icons-outlined">picture_as_pdf</span>
                                                    <span>PDF</span>
                                                </button>
                                                <button type="button" className="purchase-return__action-btn" onClick={handleExportExcel}>
                                ...
                                <div className="purchase-return__body">
                                    <form ref={returnRef} onSubmit={handleSubmit} className="purchase-return__form">

                                        <div className="purchase-return__card">
                                            <h3 className="purchase-return__section-title">
                                                <span className="material-icons-outlined">description</span>
                                                {t('document_information', 'Document Information')}
                                            </h3>
                                            <div className="purchase-return__section-body">
                                                <div className="purchase-return__grid purchase-return__grid--doc">
                                                    <div className="purchase-return__field">
                                                        <label className="purchase-return__label">{t('return_number', 'Return #')}</label>
                                                        <input type="text" value={data.return_number} disabled placeholder={t('auto_generated', 'Auto-generated')} />
                                                    </div>
                                                    <div className="purchase-return__field">
                                                        <label className="purchase-return__label">{t('return_date', 'Return Date')} <span className="purchase-return__required">*</span></label>
                                                        <input type="date" value={data.return_date} onChange={e => setData('return_date', e.target.value)} readOnly={formDisabled} />
                                                    </div>
                                                    <div className="purchase-return__field">
                                                        <label className="purchase-return__label">{t('return_type', 'Return Type')}</label>
                                                    ...
                                                    <div className="purchase-return__field">
                                                        <label className="purchase-return__label">{t('status', 'Status')}</label>
                                                    ...
                                                    <div className="purchase-return__field">
                                                        <label className="purchase-return__label">{t('refund_status', 'Refund Status')}</label>
                                        ...
                                        <div className="purchase-return__card">
                                            <h3 className="purchase-return__section-title">
                                                ...
                                                {t('supplier_and_invoice', 'Supplier & Invoice')}
                                                {selectedInvoice ? (
                                                    <span className="purchase-return__section-hint">
                                                        <span className="material-icons-outlined" style={{ fontSize: '14px', color: 'var(--pr-success)', marginInlineEnd: '4px' }}>check_circle</span>
                                                        {t('hint_invoice_loaded', 'Loaded from:')}{' '}
                                                        <strong style={{ color: 'var(--pr-text-secondary)' }}>
                                                            {selectedInvoice.invoice_number || `#${selectedInvoice.id}`}
                                                        </strong>
                                                        {(selectedInvoice.supplier_name_en || selectedInvoice.supplier_name_ar) && (
                                                            <>
                                                                {' · '}
                                                                {selectedInvoice.supplier_name_en || selectedInvoice.supplier_name_ar}
                                                            </>
                                                    ...
                                                    <span className="purchase-return__section-hint">
                                                        {t('hint_invoice_source', 'Select a Purchase Invoice first. All other fields will sync automatically.')}
                                                    </span>
                                                )}
                                            </h3>
                                            <div className="purchase-return__section-body">
                                                <div className="purchase-return__grid purchase-return__grid--party">
                                                    <div className="purchase-return__field">
                                                        <label className="purchase-return__label">
                                                            {t('purchase_invoice', 'Purchase Invoice')} <span className="purchase-return__required">*</span>
                                                        </label>
                                                        <div className={isInvoiceLoading ? 'purchase-return__loading-wrap' : ''}>
                                                            <SearchableComboBox
                                                                options={invoiceOptions}
                                                                value={data.invoice_id ? String(data.invoice_id) : ''}
                                                                onChange={(val) => !formDisabled && handleInvoiceChange(val)}
                                                                disabled={formDisabled}
                                                                placeholder={t('select_invoice', 'Select Purchase Invoice...')}
                                                            />
                                                            {isInvoiceLoading && (
                                                                <span className="purchase-return__loading-indicator">
                                                                    <span className="material-icons-outlined purchase-return__spin">sync</span>
                                                                    {t('loading_invoice', 'Loading invoice data...')}
                                                                </span>
                                                            )}
                                                        </div>
                                                    </div>
                                                    <div className="purchase-return__field" {...renderDisabledTooltip(invoiceLocked ? t('supplier_locked_reason', 'Supplier is locked because it must match the Purchase Invoice') : '')}>
                                                        <label className="purchase-return__label">
                                                            {t('supplier', 'Supplier')} <span className="purchase-return__required">*</span>
                                                            {invoiceLocked && <span className="purchase-return__lock-icon" title={t('supplier_locked_reason', 'Locked to match Purchase Invoice')}>🔒</span>}
                                                        </label>
                                                        <SearchableComboBox
                                                            ...
                                                            placeholder={invoiceLocked ? '' : t('supplier_loaded_automatically', 'Loaded when Invoice is selected')}
                                                        />
                                                    </div>
                                                    <div className="purchase-return__field" {...renderDisabledTooltip(invoiceLocked ? t('warehouse_locked_reason', 'Warehouse is locked because returns must go back to the original receipt warehouse') : '')}>
                                                        <label className="purchase-return__label">
                                                            {t('warehouse', 'Warehouse')} <span className="purchase-return__required">*</span>
                                                            {invoiceLocked && <span className="purchase-return__lock-icon" title={t('warehouse_locked_reason', 'Locked to match Purchase Invoice warehouse')}>🔒</span>}
                                                        </label>
                                                        <SearchableComboBox
                                        ...
                                        <div className="purchase-return__card">
                                            ...
                                            <div className="purchase-return__section-body">
                                                <div className="purchase-return__grid purchase-return__grid--return">
                                                    <div className="purchase-return__field purchase-return__span-full">
                                                        <label className="purchase-return__label">{t('return_reason', 'Return Reason')} <span className="purchase-return__required">*</span></label>
                                                        <select value={data.return_reason} onChange={e => setData('return_reason', e.target.value)} disabled={formDisabled} className="purchase-return__select">
                                                    ...
                                                    <div className="purchase-return__field">
                                                        <label className="purchase-return__label">{t('received_by', 'Received By')}</label>
                                                        <input type="number" value={data.received_by} onChange={e => setData('received_by', e.target.value)} readOnly={formDisabled} />
                                                    </div>
                                                    <div className="purchase-return__field">
                                                        <label className="purchase-return__label">{t('received_date', 'Received Date')}</label>
                                        ...
                                        <div className="purchase-return__items">
                                            <div className="purchase-return__items-header">
                                                <h3>
                                                    <span className="material-icons-outlined">inventory_2</span>
                                                    {t('returned_items', 'Returned Items')}
                                                </h3>
                                                <div className="purchase-return__items-header-hint">
                                                    {!invoiceLocked
                                                        ? t('hint_no_invoice_items', 'Select a Purchase Invoice above to load returnable items.')
                                                        : t('hint_invoice_items_locked', 'Items are loaded from Invoice. Edit only Return Qty and Condition.')}
                                                </div>
                                            </div>
                                            <div className="purchase-return__table-wrap">
                                                <table className="purchase-return__table">
                                                    <thead>
                                                        <tr>
                                                            <th className="purchase-return__table__col-num">#</th>
                                                            <th className="purchase-return__table__col-item">{t('item', 'Item')}</th>
                                                            <th className="purchase-return__table__col-qty">{t('invoice_qty', 'Invoice Qty')}</th>
                                                            <th className="purchase-return__table__col-qty">{t('prev_returned', 'Prev. Returned')}</th>
                                                            <th className="purchase-return__table__col-qty">{t('available_qty', 'Available')}</th>
                                                            <th className="purchase-return__table__col-qty">{t('return_qty', 'Return Qty')}*</th>
                                                            <th className="purchase-return__table__col-unit">{t('unit', 'Unit')}</th>
                                                            <th className="purchase-return__table__col-price">{t('price', 'Price')}</th>
                                                            <th className="purchase-return__table__col-pct">{t('tax_pct', 'Tax %')}</th>
                                                            <th className="purchase-return__table__col-tax">{t('tax_amount', 'Tax')}</th>
                                                            <th className="purchase-return__table__col-cond">{t('condition', 'Condition')}</th>
                                                            <th className="purchase-return__table__col-batch">{t('batch', 'Batch')}</th>
                                                            <th className="purchase-return__table__col-serial">{t('serial', 'Serial')}</th>
                                                            <th className="purchase-return__table__col-total">{t('total', 'Total')}</th>
                                                            <th className="purchase-return__table__col-action"></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        {data.items.length === 0 && (
                                                            <tr className="is-empty-row">
                                                                <td colSpan={15}>
                                                                    <div className="purchase-return__empty-state">
                                                                        <span className="material-icons-outlined">assignment_return</span>
                                                                        <p>
                                                                            {!invoiceLocked
                                                                                ? t('empty_items_hint', 'Select a Purchase Invoice above to automatically load returnable items.')
                                                        ...
                                                        {data.items.map((item, index) => {
                                                            ...
                                                            return (
                                                                <tr key={index} className={qtyWarning ? 'purchase-return__row--warn' : ''}>
                                                                    <td className="purchase-return__table__col-num">{index + 1}</td>
                                                                    <td className="purchase-return__table__col-item">
                                                                        <div className="purchase-return__item-name">
                                                                            <input
                                                                                type="text"
                                                                                value={item.item_name_en || item.item_name_ar || products.find(p => p.id == item.product_id)?.name_en || ''}
                                                                                readOnly
                                                                                className="purchase-return__item-name-input purchase-return__read-only"
                                                                                title={t('item_locked', 'Item is locked from the Purchase Invoice')}
                                                                            />
                                                                            <input
                                                                                type="text"
                                                                                className="purchase-return__item-desc"
                                                                    ...
                                                                    <td className="purchase-return__table__col-qty">
                                                                        <input type="number" value={Number(item.invoice_qty).toFixed(2)} readOnly className="purchase-return__read-only" title={t('invoice_qty_locked', 'Invoice quantity - locked')} />
                                                                    </td>
                                                                    <td className="purchase-return__table__col-qty">
                                                                        <input type="number" value={Number(item.returned_qty).toFixed(2)} readOnly className="purchase-return__read-only" title={t('returned_qty_locked', 'Already returned on other documents')} />
                                                                    </td>
                                                                    <td className="purchase-return__table__col-qty">
                                                                        <input type="number" value={Number(item.available_qty).toFixed(2)} readOnly className="purchase-return__read-only" title={t('available_qty_locked', 'Remaining quantity available to return')} />
                                                                    </td>
                                                                    <td className="purchase-return__table__col-qty">
                                                                        <input
                                                                            type="number"
                                                                            ...
                                                                            className={qtyWarning ? 'purchase-return__input--warn' : ''}
                                                                            title={rq <= 0 ? t('hint_enter_return_qty', 'Enter return quantity > 0 to include this line') : ''}
                                                                        />
                                                                    </td>
                                                                    <td className="purchase-return__table__col-unit">
                                                                        <select
                                                                            value={item.unit_id ? String(item.unit_id) : ''}
                                                                            onChange={() => {}}
                                                                            disabled={true}
                                                                            className="purchase-return__read-only"
                                                                            title={t('unit_locked_reason', 'Unit is locked from Purchase Invoice')}
                                                                        >
                                                                    ...
                                                                    <td className="purchase-return__table__col-price">
                                                                        <input
                                                                            type="number"
                                                                            value={Number(item.unit_price).toFixed(4)}
                                                                            readOnly
                                                                            className="purchase-return__read-only"
                                                                            title={t('price_locked_reason', 'Price is locked from Purchase Invoice')}
                                                                        />
                                                                    </td>
                                                                    <td className="purchase-return__table__col-pct">
                                                                        <input
                                                                            type="number"
                                                                            value={Number(item.tax_percentage).toFixed(2)}
                                                                            readOnly
                                                                            className="purchase-return__read-only"
                                                                            title={t('tax_locked_reason', 'Tax % is locked from Purchase Invoice')}
                                                                        />
                                                                    </td>
                                                                    <td className="purchase-return__table__col-tax">
                                                                        <input type="text" value={Number(item.tax_amount).toFixed(2)} readOnly />
                                                                    </td>
                                                                    <td className="purchase-return__table__col-cond">
                                                                        <select
                                                                    ...
                                                                    <td className="purchase-return__table__col-batch">
                                                                        <input
                                                                            type="text"
                                                                            value={item.batch_number}
                                                                            readOnly
                                                                            className="purchase-return__read-only"
                                                                            title={t('batch_locked_reason', 'Batch # is copied from Purchase Invoice')}
                                                                        />
                                                                    </td>
                                                                    <td className="purchase-return__table__col-serial">
                                                                        <input
                                                                            type="text"
                                                                            value={item.serial_number}
                                                                            readOnly
                                                                            className="purchase-return__read-only"
                                                                            title={t('serial_locked_reason', 'Serial # is copied from Purchase Invoice')}
                                                                        />
                                                                    </td>
                                                                    <td className="purchase-return__table__col-total">
                                                                        <input type="text" value={Number(item.line_total).toFixed(2)} readOnly />
                                                                    </td>
                                                                    <td className="purchase-return__table__col-action">
                                                                        {!formDisabled && (
                                                                            <button
                                                                                type="button"
                                                                                className="purchase-return__row-del"
                                        ...
                                        <div className="purchase-return__footer-grid">
                                            <div className="purchase-return__notes-col">
                                                <div className="purchase-return__card">
                                                    <h3 className="purchase-return__section-title">
                                                    ...
                                                    <div className="purchase-return__section-body">
                                                        <div className="purchase-return__field">
                                                            <textarea className="purchase-return__textarea" value={data.approval_notes} onChange={e => setData('approval_notes', e.target.value)} readOnly={formDisabled} />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="purchase-return__card">
                                                    ...
                                                    <div className="purchase-return__section-body">
                                                        <div className="purchase-return__field">
                                                            <textarea className="purchase-return__textarea purchase-return__textarea--lg" value={data.notes} onChange={e => setData('notes', e.target.value)} readOnly={formDisabled} />
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>

                                            <div className="purchase-return__summary">
                                                <div className="purchase-return__summary-head">
                                                    <h3>
                                                        <span className="material-icons-outlined">payments</span>
                                                        {t('financial_summary', 'Financial Summary')}
                                                    </h3>
                                                </div>
                                                <div className="purchase-return__summary-body">
                                                    <div className="purchase-return__summary-row">
                                                        <span className="purchase-return__summary-label">{t('subtotal', 'Subtotal')}</span>
                                                        <span className="purchase-return__summary-value purchase-return__mono">{Number(data.subtotal).toFixed(2)}</span>
                                                    </div>
                                                    <div className="purchase-return__summary-row">
                                                        <span className="purchase-return__summary-label">{t('tax_total', 'Tax Total')}</span>
                                                        <span className="purchase-return__summary-value purchase-return__mono">{Number(data.tax_amount).toFixed(2)}</span>
                                                    </div>
                                                    <div className="purchase-return__summary-row purchase-return__summary-row--subtle">
                                                        <span className="purchase-return__summary-label">{t('restocking_fee', 'Restocking Fee')}</span>
                                                    ...
                                                    <div className="purchase-return__summary-row purchase-return__summary-row--total">
                                                        <span className="purchase-return__summary-label">{t('grand_total', 'Total Amount')}</span>
                                                        <span className="purchase-return__summary-value purchase-return__mono">{Number(data.total_amount).toFixed(2)}</span>
                                                    </div>
                                                    <div className="purchase-return__summary-row purchase-return__summary-row--refund">
                                                        <span className="purchase-return__summary-label">{t('refund_amount', 'Refund Amount')}</span>
                                                        <span className="purchase-return__summary-value purchase-return__mono">{Number(data.refund_amount).toFixed(2)}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                {!isViewMode && (
                                    <div className="purchase-return__footer">
                                        <button type="button" className="purchase-return__btn purchase-return__btn--ghost" onClick={() => setMode('list')} disabled={processing}>
                                            <span className="material-icons-outlined">cancel</span>
                                            {t('common.cancel', 'Cancel')}
                                        </button>
                                        <button type="button" className="purchase-return__btn purchase-return__btn--primary" onClick={handleSubmit} disabled={processing || !invoiceLocked}>
                            ...
                            <div className="purchase-return__print" ref={printRef}>
                                <div className="pr-print__company">
                                    <div className="pr-print__company-brand">
                                        <h1>ZODIC ERP</h1>
                                        <p>123 Business Street, City, Country</p>
                                        <p>Phone: +1 234 567 890</p>
                                        <p>Email: info@zodicerp.com</p>
                                    </div>
                                    <div className="pr-print__company-doc">
                                        <h2>{t('purchase_return', 'Purchase Return').toUpperCase()}</h2>
                                        <div className="pr-print__meta-item pr-print__meta-item--end pr-print__meta-item--mt1">
                                            <span className="label">{t('return_number', 'Return #')}:</span>
                                            <span className="value pr-print__value--strong">{data.return_number || 'DRAFT'}</span>
                                        </div>
                                        <div className="pr-print__meta-item pr-print__meta-item--end pr-print__meta-item--mt05">
                                ...
                                <div className="pr-print__meta-grid">
                                    <div className="pr-print__meta-item">
                                        <span className="label">{t('supplier', 'Supplier')}</span>
                                        <span className="value">{printSupplierName}</span>
                                    </div>
                                    <div className="pr-print__meta-item">
                                        <span className="label">{t('purchase_invoice', 'Purchase Invoice')}</span>
                                ...
                                <table className="pr-print__table">
                                    <thead>
                                        <tr>
                                            <th className="pr-print__th-num">#</th>
                                            <th>{t('description', 'Description')}</th>
                                            <th className="pr-print__th-qty ctr">{t('return_qty', 'Qty')}</th>
                                            <th className="pr-print__th-price num">{t('price', 'Price')}</th>
                                            <th className="pr-print__th-tax num">{t('tax_amount', 'Tax')}</th>
                                            <th className="pr-print__th-total num">{t('total', 'Total')}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        {data.items.filter(i => parseFloat(i.return_qty || 0) > 0).length === 0 && (
                                            <tr>
                                                <td colSpan={6} className="pr-print__empty-row">
                                ...
                                <div className="pr-print__totals">
                                    <div className="pr-print__totals-row">
                                        <span className="pr-print__totals-label">{t('subtotal', 'Subtotal')}:</span>
                                        <span className="pr-print__totals-value">{Number(data.subtotal).toFixed(2)}</span>
                                    </div>
                                    <div className="pr-print__totals-row">
                                        <span className="pr-print__totals-label">{t('tax_total', 'Tax')}:</span>
                                        <span className="pr-print__totals-value">{Number(data.tax_amount).toFixed(2)}</span>
                                    </div>
                                    {parseFloat(data.restocking_fee || 0) > 0 && (
                                        <div className="pr-print__totals-row pr-print__totals-row--subtle">
                                            <span className="pr-print__totals-label">{t('restocking_fee', 'Restocking Fee')}:</span>
                                            <span className="pr-print__totals-value">-{Number(data.restocking_fee).toFixed(2)}</span>
                                        </div>
                                    )}
                                    <div className="pr-print__totals-row pr-print__totals-row--total">
                                        <span className="pr-print__totals-label">{t('grand_total', 'Grand Total')}:</span>
                                        <span className="pr-print__totals-value pr-print__totals-value--strong">{Number(data.total_amount).toFixed(2)}</span>
                                    </div>
                                    <div className="pr-print__totals-row pr-print__totals-row--refund">
                                        <span className="pr-print__totals-label">{t('refund_amount', 'Refund Amount')}:</span>
                                        <span className="pr-print__totals-value pr-print__totals-value--strong">{Number(data.refund_amount).toFixed(2)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
