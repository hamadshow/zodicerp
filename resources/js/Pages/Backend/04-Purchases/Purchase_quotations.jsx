...
export default function PurchaseQuotations({ quotations, vendors, currencies, products, units, paymentTerms, deliveryTerms }) {
    ...
    const handleExportPDF = () => {
        ...
        const module = element.closest('.purchase-quotations-module');
        if (module) module.classList.add('generating-pdf');
        
        const opt = {
            margin: [5, 5],
            filename: `PurchaseQuotation_${data.quotation_number || 'New'}.pdf`,
    ...
    const handleExportExcel = () => {
        ...
        XLSX.writeFile(workbook, `PurchaseQuotation_${data.quotation_number || 'New'}.xlsx`);
    };

    const breadcrumbs = [
        { label: 'Dashboard', href: route('admin.dashboard') },
        { label: 'Purchases', href: '#' },
        { label: 'Purchase Quotations', onClick: () => setMode('list') }
    ];

    if (mode === 'create') breadcrumbs.push({ label: 'New Quotation' });
    if (mode === 'edit') breadcrumbs.push({ label: `Edit Quotation ${data.quotation_number || ''}` });

    const filtersContent = (
        <div className="purchase-quotations-module__header">
            <h1>
                {mode === 'list' ? 'Purchase Quotations' : 
                            ...
                            <div className="print-totals">
                                <div className="totals-box">
                                    ...
                                    <div className="row grand-total">
                                        ...
                                        <span>{Number(data.grand_total).toFixed(2)} {currencies.find(c => c.id == data.currency_id)?.code}</span>
                                    </div>
                                </div>
                            </div>
