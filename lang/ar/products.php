<?php

return [
    'products' => 'المنتجات',
    'inventory' => 'المخزون',
    'dashboard' => 'لوحة التحكم',
    'products_list' => 'قائمة المنتجات',
    'add_product' => 'إضافة منتج',
    'add_new_product' => 'إضافة منتج جديد',
    'edit_product' => 'تعديل منتج',
    'search' => 'بحث...',
    'filter' => 'تصفية',
    'product' => 'المنتج',
    'name' => 'اسم المنتج',
    'sku' => 'كود المنتج',
    'category' => 'القسم',
    'brand' => 'العلامة التجارية',
    'unit' => 'الوحدة',
    'price' => 'السعر',
    'sale_price' => 'سعر البيع',
    'cost_price' => 'سعر التكلفة',
    'qty' => 'الكمية',
    'stock' => 'المخزون',
    'status' => 'الحالة',
    'actions' => 'العمليات',
    'no_products_found' => 'لم يتم العثور على منتجات.',
    'all_categories' => 'جميع الأقسام',
    'all_brands' => 'جميع العلامات التجارية',
    'all_units' => 'جميع الوحدات',
    'all_status' => 'جميع الحالات',
    'active' => 'نشط',
    'inactive' => 'غير نشط',
    'draft' => 'مسودة',
    'total_products' => 'إجمالي المنتجات',
    'total_categories' => 'إجمالي الأقسام',
    'total_brands' => 'إجمالي العلامات التجارية',
    'total_value' => 'القيمة الإجمالية',
    'import_excel' => 'استيراد من إكسل',
    'export_excel' => 'تصدير إلى إكسل',
    'download_template' => 'تحميل النموذج',
    'excel' => 'إكسل',
    'edit' => 'تعديل',
    'delete' => 'حذف',
    'confirm_delete' => 'هل أنت متأكد من حذف هذا المنتج؟',
    'desc' => 'الوصف',
    'image' => 'صورة المنتج',
    'save' => 'حفظ',
    'save_and_exit' => 'حفظ وخروج',
    'cancel' => 'إلغاء',
    'back_to_list' => 'العودة للقائمة',
    'product_type' => 'نوع المنتج',
    'simple' => 'بسيط',
    'variable' => 'متغير',
    'service' => 'خدمة',
    'simple_product' => 'منتج بسيط',
    'variable_product' => 'منتج متغير',
    'service_product' => 'خدمة',
    'variant_sku' => 'منتج فرعي',
    'child_sku_hint' => 'منتج فرعي (SKU) تابع لمنتج قابل للتخصيص — قابل للبيع بمفرده.',
    'variable_parent' => 'متغير (أب)',
    'variable_parent_hint' => 'منتج أب قابل للتخصيص — غير قابل للبيع مباشرة. بِع أحد منتجاته الفرعية.',
    'barcode' => 'باركود',
    'stock_status' => 'حالة المخزون',
    'in_stock' => 'متوفر',
    'out_of_stock' => 'غير متوفر',
    'on_backorder' => 'طلب مسبق',
];
